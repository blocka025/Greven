...
save_file.write("Time (min)" + "\t" + "T (K)" + "\t" + 'Freq (kHz)' + "\t"+ "Vx (V)" + "\t" + "Vy (V)"+ "\t" + "Voltage Mode"+"\n")
#votlage mode: -1 means A-B mode, 1 means A

set_temp = 92
signal_amp = 500 #in mV
...
if ls.query('Range? 1') != '1' or abs(float(ls.query('KRDG? a'))-set_temp)>.1: #this ignores this if already stable
    ls.write('Range 1,1')
    time.sleep(0.1)
...
while True:
    s = input('Full FRA? (y/n): ')
    if s =='y':
        b = True
        print('Time Estimate = '+str(10.5*len(freqs)*trials*2/60)+'min')
        break
    elif s=='n':
        b = False
        print('Time Estimate = '+str(10.5*len(freqs)*trials/60)+'min')
        break
...
for i in range(trials):
    ...
    for freq in freqs:
        ...
        values['Freq'] = freq
        save_file.write(str(values['time']) + "\t" +  str(values['Temp']) + "\t" + str(values['Freq']) + "\t" + str(values['Vx']) + "\t" + str(values['Vy'])+ "\t"+'-1'+"\n")
        save_file.flush()
if b:
    srs.query('ISRC 0')
    for i in range(trials):
        print('Begin Second Trials Part ' + str(i+1))
        for freq in freqs:
            try:
                srs.query('FREQ '+ str(freq) + ' KHZ')
            except:
