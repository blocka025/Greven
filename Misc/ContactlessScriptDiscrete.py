...
def get_parameters(f):
    try:
        ...
        return (lines[0].split()[1],#WaitTime
...
ls.write('RAMP 1,0,1') #the ramping is off
time.sleep(0.05)
ls.write('SETP 1,'+ parameters[1]) #this is the starting temp
time.sleep(0.05)
ls.write('CMODE,1,1') #this put the PID in manual mode
time.sleep(0.05)
ls.write('Range 1,1') #this turns the heater on
...
while parameters[3] < 2:
    ...
    if parameters[3] == 0: #Constant Temp Mode
        ls.write('RAMP 1,0,'+ parameters[0])# Turns off ramping
        time.sleep(0.15)
        ls.write('SETP 1,'+ parameters[1])# sets base temp
        
    elif parameters[3] == 1: #ramp mode
        ls.write('PID 1,'+ parameters[4][0]+','+ parameters[4][1]+',' + parameters[4][2])#this sets the PID parameters
        time.sleep(0.05)
        if stable:
            
            if t2 - t1 >parameters[0]:
...
# Close Instruments
#######################
ls.write('SETP 1,'+ parameters[1]) #set temp back to base
time.sleep(0.05)
